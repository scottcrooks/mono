...
func printUsage() {
	...
	fmt.Println("  <task> [--no-cache] [--concurrency N] [--integration] [service...]")
	fmt.Println("                        Run orchestrated tasks across services (build/lint/typecheck/test/audit/package/deploy)")
	fmt.Println("  <command>             Run custom command across services")
	fmt.Println()
	fmt.Println("Examples:")
	fmt.Println("  mono test             Run tests for all services")
	fmt.Println("  mono audit            Run security scanning for all services")
	fmt.Println("  mono test pythia      Run tests for pythia only")
	fmt.Println("  mono test --integration pythia  Run integration tests for pythia")
	fmt.Println("  mono affected         List impacted services from current branch changes")
	fmt.Println("  mono affected --explain  Show dependency chains for impacted services")
	fmt.Println("  mono check --base main  Run pending PR checks for impacted services")
	fmt.Println("  mono status --base main  Show changed/impacted summary against main")
