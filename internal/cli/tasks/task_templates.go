...
func TaskCommandForServiceWithOptions(svc Service, task TaskName, integration bool) (string, bool, string) {
	if integration {
		...
		case "react":
			...
			// Force non-interactive reporter for CLI task runs so the command exits
			// instead of waiting on Playwright's HTML report server.
			return "pnpm test:integration -- --reporter=line", true, ""
		case "":
